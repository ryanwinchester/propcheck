defmodule PropCheck.Test.PingPongStateM do
  ...
  def command(%__MODULE__{players: []}), do: {:call, PingPongMaster, :add_player, [name()]}
  ...
  def next_state(s, _value, {:call, PingPongMaster, :add_player, [name]}) do
    case s.players |> Enum.member?(name) do
      false -> %__MODULE__{s | players: [name | s.players], scores: s.scores |> Map.put(name, 0)}
      true -> s
    end
  end

  def next_state(s, _value, {:call, PingPongMaster, :remove_player, [name]}) do
    %__MODULE__{s | players: s.players |> List.delete(name), scores: s.scores |> Map.delete(name)}
  end

  def next_state(s, _value, {:call, PingPongMaster, :play_ping_pong, [name]}) do
    %__MODULE__{s | scores: s.scores |> Map.update!(name, &(&1 + 1))}
  end

  def next_state(state, _value, _call), do: state

  @doc "can the call in the current state be made?"
  def precondition(s, {:call, PingPongMaster, do_it, [name]})
      when do_it in [:remove_player, :play_tennis, :play_ping_pong, :get_score] do
  ...
  """
  def postcondition(_s, {:call, PingPongMaster, :remove_player, [name]}, {:removed, n}),
    do: n == name
  ...
  def postcondition(_s, {:call, PingPongMaster, :play_ping_pong, [name]}, {:dead_player, name}),
    do: true

  def postcondition(_s, {:call, PingPongMaster, :play_tennis, [name]}, {:dead_player, name}),
    do: true

  def postcondition(_s, {:call, PingPongMaster, :play_football, [name]}, {:dead_player, name}),
    do: true

  def postcondition(s, {:call, PingPongMaster, :get_score, [name]}, result),
    ...
    do: result <= s.scores |> Map.fetch!(name)

  def postcondition(s, {:call, _, f, _}, r) do
    IO.puts(
      "Failing postcondition for call #{f} with result #{inspect(r)} in state #{inspect(s)}"
    )
