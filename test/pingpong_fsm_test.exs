defmodule PropCheck.Test.PingPongFSM do
  ...
  defp log_message(log_state, {:in, msg, client}, proc_state) do
    Logger.debug(
      "Got message #{inspect(msg, :pretty)} from client #{inspect(client)} in state #{
        inspect(proc_state, :pretty)
      }"
    )
  ...
  defp log_message(log_state, {:out, msg, client}, proc_state) do
    Logger.error(
      "Send message #{inspect(msg, :pretty)} to client #{inspect(client)} in state #{
        inspect(proc_state, :pretty)
      }"
    )
