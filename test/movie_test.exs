defmodule PropCheck.Test.Movies do
  ...
  def command(_state = %__MODULE__{users: []}) do
    frequency([
      {1, {:call, MovieServer, :create_account, [name()]}},
  ...
  def next_state(s = %__MODULE__{users: users}, v, {:call, _, :create_account, [_name]}),
    do: %__MODULE__{s | users: [v | users]}

  def next_state(
        s = %__MODULE__{rented: rented, users: users},
        _v,
        {:call, _, :delete_account, [password]}
      ) do
    case Map.get(rented, password, []) do
      [] ->
        %__MODULE__{
          s
          | users: List.delete(users, password),
  ...
  def next_state(s = %__MODULE__{rented: rented}, _v, {:call, _, :return_dvd, [password, movie]}),
    do: %__MODULE__{s | rented: Map.update!(rented, password, &(&1 |> List.delete(movie)))}
